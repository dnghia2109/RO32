...
public class OrderService {
    ...
    public void updateStatusOrder(Order order, long id) {
        ...
        for (OrderItem oi : orderItems) {
            ...
            if (order1.getStatus() == RETURNED_STATUS || order1.getStatus() == CANCELED_STATUS) {
                productRepository.plusOneProductBack(oi.getProduct().getId(), oi.getQuantity());
                productRepository.minusOneProductTotalSold(oi.getProduct().getId(), oi.getQuantity());
            }
        }
        try {
